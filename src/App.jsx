...
function App() {
  ...
  const fetchUnits = async () => {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 2; attempt++) {
        ...
        const response = await axios.get('http://sm-api2.runasp.net/api/admin/response-units/List', {
  ...
  const deleteUnit = async (id) => {
    ...
    try {
      const response = await axios.delete(`http://sm-api2.runasp.net/api/admin/response-units/Delete/${id}`, { headers: { Authorization: `Bearer ${token}` } });
    ...
    try {
      await axios.delete(`http://sm-api2.runasp.net/api/AppUser/Delete/${id}`, { headers: { Authorization: `Bearer ${token}` } });

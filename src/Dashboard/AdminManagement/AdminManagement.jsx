...
const AdminManagement = () => {
  ...
  const fetchRoles = async () => {
    try {
      setLoading(true);
      const response = await axios.get('http://sm-api2.runasp.net/api/Role/GetAll', {
  ...
  const handleDelete = async (id, roleName) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`http://sm-api2.runasp.net/api/Role/Delete/${id}`, {
  ...
  const handleEdit = async (role) => {
    ...
    if (newName) {
      try {
        await axios.put(`http://sm-api2.runasp.net/api/Role/Edit`, {
          id: role.id,
          newRoleName: newName 
        }, {
          headers: { 'Authorization': `Bearer ${token}` }
